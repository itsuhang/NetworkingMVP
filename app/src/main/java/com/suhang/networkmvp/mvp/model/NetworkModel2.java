...
public class NetworkModel2 extends BaseModel {
    @Inject
     ...
     */
    public void loadPostData(boolean isWrap, Class<? extends ErrorBean> aClass, String append, Map<String, String> params, boolean needCache, int tag) {
        if (isWrap) {
     ...
     * @param tag 标记,用于一个页面同时处理多个获取数据的请求
     */
    public void loadPostData(boolean isWrap, Class<? extends ErrorBean> aClass, String append, Map<String, String> params, String cacheTag, int tag) {
        if (isWrap) {
     ...
     * @param tag 标记,用于一个页面同时处理多个获取数据的请求
     */
    public void loadPostData(boolean isWrap, Class<? extends ErrorBean> aClass, Map<String, String> params, String cacheTag, int tag) {
        if (isWrap) {
     ...
     * @param tag 标记,用于一个页面同时处理多个获取数据的请求
     */
    public void loadPostData(boolean isWrap, Class<? extends ErrorBean> aClass, Map<String, String> params, boolean needCache, int tag) {
        if (isWrap) {
     ...
     * @param tag 标记,用于一个页面同时处理多个获取数据的请求
     */
    public void loadGetData(boolean isWrap, Class<? extends ErrorBean> aClass, String path, Map<String, String> params, int tag) {
        if (isWrap) {
        ...
        if (flowable != null) {
            Disposable disposable = flowable.compose(RxUtil.fixScheduler()).subscribe(o -> {
                if (o.getCode() != null) {
                    ...
                    mRxBus.post(new SuccessResult(o, tag, true));
                }
            }, throwable -> {
    ...
    public void upload(Class<? extends ErrorBean> aClass, Map<String, String> params, File file, int tag) {
        ...
        if (flowable != null) {
            Disposable disposable = flowable.compose(RxUtil.fixScheduler()).subscribe(o -> mRxBus.post(new SuccessResult(o, tag, false)), throwable -> {
                ErrorBean errorBean = new ErrorBean(ErrorCode.ERROR_CODE_NETWORK, ErrorCode.ERROR_DESC_NETWORK);
                errorBean.setType(Constants.ERRORTYPE_TWO);
                LogUtil.i("啊啊啊" + errorBean.getCode() + "   " + errorBean.getDesc() + "  " + throwable);
                mRxBus.post(new ErrorResult(errorBean, tag));
    ...
    private void dealCache(Class<? extends ErrorBean> aClass, String append, String cacheTag, ErrorBean o, int tag) {
        mDisposable.add(Flowable.create((FlowableOnSubscribe<ErrorBean>) sub -> {
            ...
            if (o == null) {
                mRxBus.post(new SuccessResult(errorBean, tag, false));
            }
        }));
    }
    ...
    private File saveFile(Response response, String destFileDir, String name, int tag) throws IOException {
        ...
        mRxBus.post(new SuccessResult(new DownLoadBean(file.getAbsolutePath()), tag, false));
        fos.flush();
        is.close();
        fos.close();
        return file;
